...
class cGameGUI : public cMenu
{
	...
	*/
	...
	bool checkScroll(cMouse& mouse);
	void updateUnderMouseObject(const cMouse& mouse);
	...
	*@author alzi alias DoctorDeath
	*/
	void selectBoxVehicles(const cBox<cPosition>& box);
	/**
	...
	*/
	...
	bool areCoordsOverMenu (const cUnit& unit, const cPosition& position) const;
	...
	void screenShotTaken(const std::string& fileName);
protected:

	virtual void activate() MAXR_OVERRIDE_FUNCTION;
	virtual void deactivate() MAXR_OVERRIDE_FUNCTION;
public:
	cGameGUI();
	~cGameGUI();
	...
	*/
	...
	/**
	 * Returns the map tile position of the passed screen position.
	 *
	 * @param screenPosition The pixel screen position to get the tile position for.
	 * @return The position of the map tile that is under the given screen position.
	 *         If the screen position is not above the map the position (-1, -1) is returned.
	 */
	cPosition getTilePosition(const cPosition& screenPosition) const;
};

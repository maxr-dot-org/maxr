...
void cEngine::AddMoveJob ( cMJobs *job )
{
	// Server/SP:
	SDL_LockMutex(mutex);
	job->next=mjobs;
	...
	if ( job->vehicle->IsBuilding&&!job->finished )
	{
		...
		if( job->vehicle->owner == game->ActivePlayer )
		{
	...
	if ( job&&job->vehicle->BuildPath&&job->vehicle->data.can_build==BUILD_SMALL&& ( job->vehicle->BandX!=job->vehicle->PosX||job->vehicle->BandY!=job->vehicle->PosY ) &&!job->finished&&job->vehicle->data.cargo>=job->vehicle->BuildCosts*job->vehicle->BuildRoundsStart )
	{
...
void cEngine::addEvent ( int iTyp, void *data1, void *data2 )
{
	switch ( iTyp )
	{
	case GAME_EVENT_WT_MOVE:
		{
			cout << "WT_MOVE\n";
			cMJobs *job = new cMJobs ( map, ((Uint32*)data1)[0], ((Uint32*)data1)[1], ((char*)data1)[8], true );

			unsigned char *senddata = ( unsigned char *) malloc ( 5 );
			((bool*)senddata)[0] = job->finished;
			sWaypoint *next;
			int iWaypoints = 0;
			while ( job->waypoints )
			{
				next = job->waypoints->next;
				senddata = (unsigned char *) realloc( senddata, 5+sizeof( sWaypoint )*(iWaypoints+1) );
				((sWaypoint*)(senddata+5))[iWaypoints].Costs = job->waypoints->Costs;
				((sWaypoint*)(senddata+5))[iWaypoints].X = job->waypoints->X;
				((sWaypoint*)(senddata+5))[iWaypoints].Y = job->waypoints->Y;
				job->waypoints = next;
				iWaypoints++;
			}
			((Uint32*)(senddata+1))[0] = iWaypoints;

			SDL_Event SDLEvent;
			SDLEvent.type = ENGINE_EVENT;
			SDLEvent.user.code = GAME_EVENT_OK_MOVE;
			SDLEvent.user.data1 = malloc( 5+sizeof (sWaypoint)*((Uint32*)(senddata+1))[0] );
			memcpy(SDLEvent.user.data1, senddata, 5+sizeof (sWaypoint)*((Uint32*)(senddata+1))[0] );
			SDLEvent.user.data2 = malloc( 10 );
			memcpy(SDLEvent.user.data2, data1, 10 );
			if( !network )
			{
		...
		break;
	case GAME_EVENT_OK_MOVE:
		{
			int iWaypoints = ((Uint32*)((char*)data1+1))[0];
			sWaypoint *waypoints = NULL, *next;
			cout << "OK_MOVE\n";
			if( iWaypoints > 0 )
			{
				waypoints = (sWaypoint *) malloc ( sizeof ( sWaypoint ) );
				memcpy ( waypoints, &((sWaypoint*)((char*)data1+5))[0], sizeof ( sWaypoint ) );
				next = waypoints->next = (sWaypoint *) malloc ( sizeof ( sWaypoint ) );
			}
			for ( int i = 1; i < iWaypoints; i++ )
			{
				memcpy ( next, &((sWaypoint*)((unsigned char*)data1+5))[i], sizeof ( sWaypoint ) );
				if( i != iWaypoints-1 )next = next->next = (sWaypoint *) malloc ( sizeof ( sWaypoint ) );
				else next->next = NULL;
			}
			cMJobs *job = new cMJobs ( map,((Uint32*)data2)[0],((Uint32*)data2)[1],((char*)data2)[8], false, waypoints, ((bool*)data1)[0] );
			AddMoveJob ( job );
			game->ReturnData = job;
		}
		break;

  ...
  if(IsWorking){
    ...
    if(data.can_build&&BuildList&&BuildList->Count&&owner==game->ActivePlayer){
      ...
      if(ptr->metall_remaining>0){
        static char str[50];
        int r;
        r = floor(ptr->metall_remaining/(data.metal_need*BuildSpeed*1.0));
        if(!r)r++;
...
void cBuilding::ShowTransfer(sGameObjects *target){
  ...
  while(1){
    ...
    if(x>=44+166&&x<44+223+166&&y>=86+159&&y<86+20+159&&b&&!LastB){
      PlayFX(SNDObjectMenu);
      Transf=Round((x-(44+166))*(MaxTarget/223.0)-Target,0);
      MakeTransBar(&Transf,MaxTarget,Target);
      SHOW_SCREEN
      mouse->draw(false,screen);
    }
  ...
  ScaleSurfaceAdv2(typ->img_org,typ->img,typ->img_org->w*(game->hud->Zoom/64.0),typ->img_org->h*(game->hud->Zoom/64.0));
  if(pv){
    ScaleSurfaceAdv2(pv->typ->img_org[0],pv->typ->img[0],pv->typ->img_org[0]->w*(game->hud->Zoom/64.0),pv->typ->img_org[0]->h*(game->hud->Zoom/64.0));
  }else{
    ScaleSurfaceAdv2(pb->typ->img_org,pb->typ->img,pb->typ->img_org->w*(game->hud->Zoom/64.0),pb->typ->img_org->h*(game->hud->Zoom/64.0));
  }
  Transfer=false;
}
...
void cBuilding::MakeTransBar(int *trans,int MaxTarget,int Target){
  ...
  DrawTransBar(223*((Target+*trans)/(float)MaxTarget));
}
...
void cBuilding::ShowStorageMetalBar(void){
  ...
  dest.y=106;
  scr.w=dest.w=20;
  scr.h=dest.h=115*(SubBase->Metal/(float)SubBase->MaxMetal);
  dest.y+=115-(int)(115*(SubBase->Metal/(float)SubBase->MaxMetal));
  SDL_BlitSurface(gfx_hud_stuff,&scr,buffer,&dest);
...
void cBuilding::ShowResearchSchieber(void){
  ...
  for(i=0;i<8;i++){
    ...
    scr.x=412;
    scr.y=46;
    dest.x=90+140+36.0*(((float)owner->ResearchTechs[i].working_on)/owner->ResearchCount);
    SDL_BlitSurface(gfx_hud_stuff,&scr,buffer,&dest);
  }
}
...
void cBuilding::ShowUpgrade(void){
  ...
  for(i=0;i<vehicle_anz;i++){
    ...
    ScaleSurfaceAdv2(vehicle[i].img_org[0],vehicle[i].img[0],vehicle[i].img_org[0]->w*(game->hud->Zoom/64.0),vehicle[i].img_org[0]->h*(game->hud->Zoom/64.0));
  ...
  for(i=0;i<building_anz;i++){
    ...
    ScaleSurfaceAdv2(building[i].img_org,building[i].img,building[i].img_org->w*(game->hud->Zoom/64.0),building[i].img_org->h*(game->hud->Zoom/64.0));
    n=new sUpgradeStruct;
    n->sf=sf;
    n->id=i;
    n->vehicle=false;
...
void cBuilding::ShowGoldBar(int StartCredits){
  ...
  scr.y=336;
  scr.w=dest.w=16;
  scr.h=dest.h=115*(owner->Credits/(float)StartCredits);
  dest.x=375;
...
void cBuilding::DrawMineBar(int typ,int value,int max_value,int offy,bool number,int fixed){
  ...
  if(max_value==0){
    ...
    dest.w=scr.w=(value/((float)max_value))*240;
    scr.x=156+(240-(value/((float)max_value))*240);
  }
  dest.x=174;
  SDL_BlitSurface(gfx_hud_stuff,&scr,buffer,&dest);
  if(fixed&&scr.w!=240&&max_value!=0){
    dest.w=scr.w=(fixed/((float)max_value))*240;
...
void cBuilding::DrawAttackCursor(struct sGameObjects *go,int can_attack){
  ...
  if(t){
    if(v)wc=((float)t/v->data.max_hit_points)*35;else wc=((float)t/b->data.max_hit_points)*35; 
  }else{
    wc=0;
  }
  if(v)t=v->CalcHelth(data.damage);else t=b->CalcHelth(data.damage);
  if(t){
    if(v)wp=((float)t/v->data.max_hit_points)*35;else wp=((float)t/b->data.max_hit_points)*35;
  }else{
    wp=0;
  }
...
void cBuilding::DrawMunBar(void){
  ...
  r1.w=w;
  r2.h=(r1.h=w/6)-2;
  r2.y=(r1.y=w-r1.h+GetScreenPosY())+1;
  r2.w=(((float)(r1.w-2))/data.max_ammo)*data.ammo;
...
void cBuilding::DrawHelthBar(void){
  ...
  r1.w=w;
  r2.h=(r1.h=w/6)-2;
  r2.y=(r1.y=GetScreenPosY())+1;
  r2.w=(((float)(r1.w-2))/data.max_hit_points)*data.hit_points;

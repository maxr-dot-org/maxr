...
//  x=64-(int)(x*(64-((448.0/game->map->size)<5?5:(448.0/game->map->size)))/104);
	x=64- ( int ) ( x* ( 64- ( ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) <5?5: ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) ) ) /104 );
	if ( x<1 ) x=1;
	SetZoom ( x,y );
}
...
//  if(zoom<448/game->map->size)zoom=448/game->map->size;
	if ( zoom< ( GameSettingsData.iScreenW-192 ) /game->map->size ) zoom= ( GameSettingsData.iScreenW-192 ) /game->map->size;
	if ( zoom<5 ) zoom=5;
	else if ( zoom>64 ) zoom=64;
	Zoom=zoom;
//  zoom-=((448.0/game->map->size)<5?5:(448.0/game->map->size));
	zoom-= ( int ) ( ( ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) <5?5: ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) ) );
	scr.x=0;
	scr.y=0;
	dest.x=19;
	dest.y=DestY;
...
//  zoom=106-(int)(zoom*106.0/(64-((448.0/game->map->size)<5?5:(448.0/game->map->size))));
	zoom=106- ( int ) ( zoom*106.0/ ( 64- ( ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) <5?5: ( ( GameSettingsData.iScreenW-192.0 ) /game->map->size ) ) ) );
	scr.x=132;
	scr.y=1;
	dest.x=20+zoom;
	dest.y=DestY+1;
...
//    off=(int)(((448.0/(float)lastz*64)-(448.0/(float)Zoom*64))/2);
		off= ( int ) ( ( ( ( GameSettingsData.iScreenW-192.0 ) / ( float ) lastz*64 )- ( ( GameSettingsData.iScreenW-192.0 ) / ( float ) Zoom*64 ) ) /2 );
		lastz=Zoom;

		OffX+=off;
		OffY+=off;
//    off=game->map->size*64-(int)((448.0/Zoom)*64);
		off=game->map->size*64- ( int ) ( ( ( GameSettingsData.iScreenW-192.0 ) /Zoom ) *64 );
		while ( OffX>off ) OffX--;
		while ( OffY>off ) OffY--;
		while ( OffX<0 ) OffX++;
		while ( OffY<0 ) OffY++;
...
void cHud::CheckScroll ( bool pure )
{
	...
	if ( x<=0&&y>30&&y<GameSettingsData.iScreenH-30-18 ) {mouse->SetCursor ( CPfeil4 );DoScroll ( 4 );return;}
	if ( x>=GameSettingsData.iScreenW-18&&y>30&&y<GameSettingsData.iScreenH-30-18 ) {mouse->SetCursor ( CPfeil6 );DoScroll ( 6 );return;}

	if ( ( x<=0&&y<=30 ) || ( y<=0&&x<=30 ) ) {mouse->SetCursor ( CPfeil7 );DoScroll ( 7 );return;}
	if ( ( x>=GameSettingsData.iScreenW-18&&y<=30 ) || ( y<=0&&x>=GameSettingsData.iScreenW-30-18 ) ) {mouse->SetCursor ( CPfeil9 );DoScroll ( 9 );return;}

	if ( y<=0&&x>30&&x<GameSettingsData.iScreenW-30-18 ) {mouse->SetCursor ( CPfeil8 );DoScroll ( 8 );return;}
	if ( y>=GameSettingsData.iScreenH-18&&x>30&&x<GameSettingsData.iScreenW-30-18 ) {mouse->SetCursor ( CPfeil2 );DoScroll ( 2 );return;}

	if ( ( x<=0&&y>=GameSettingsData.iScreenH-30-18 ) || ( y>=GameSettingsData.iScreenH-18&&x<=30 ) ) {mouse->SetCursor ( CPfeil1 );DoScroll ( 1 );return;}
	if ( ( x>=GameSettingsData.iScreenW-18&&y>=GameSettingsData.iScreenH-30-18 ) || ( y>=GameSettingsData.iScreenH-18&&x>=GameSettingsData.iScreenW-30-18 ) ) {mouse->SetCursor ( CPfeil3 );DoScroll ( 3 );return;}
	...
	else if ( !game->HelpActive )
	{
		...
		else if ( game->SelectedVehicle&&game->SelectedVehicle->AttackMode&&game->SelectedVehicle->owner==game->ActivePlayer&&x>=180&&y>=18&&x<GameSettingsData.iScreenW-12&&y<GameSettingsData.iScreenH-14 )
		{
		...
		else if ( game->SelectedVehicle&&game->SelectedVehicle->StealActive&&game->SelectedVehicle->owner==game->ActivePlayer&&x>=180&&y>=18&&x<GameSettingsData.iScreenW-12&&y<GameSettingsData.iScreenH-14 )
		{
		...
		else if ( game->SelectedVehicle&&game->SelectedVehicle->DisableActive&&game->SelectedVehicle->owner==game->ActivePlayer&&x>=180&&y>=18&&x<GameSettingsData.iScreenW-12&&y<GameSettingsData.iScreenH-14 )
		{
		...
		else if ( game->SelectedBuilding&&game->SelectedBuilding->AttackMode&&game->SelectedBuilding->owner==game->ActivePlayer&&x>=180&&y>=18&&x<GameSettingsData.iScreenW-12&&y<GameSettingsData.iScreenH-14 )
		{
...
//  step=game->map->size*64-(int)((448.0/Zoom)*64);
	step=game->map->size*64- ( int ) ( ( ( GameSettingsData.iScreenW-192.0 ) /Zoom ) *64 );
	if ( hud->OffX>=step ) hud->OffX=step;
	step=game->map->size*64- ( int ) ( ( ( GameSettingsData.iScreenH-32.0 ) /Zoom ) *64 );

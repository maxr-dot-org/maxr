...
void cVehicle::FindNextband (cGameGUI& gameGUI)
{
	...
	const auto mouseTilePosition = gameGUI.getTilePosition(cMouse::getInstance().getPosition());
	...
	if(mouseTilePosition.x() <= PosX && mouseTilePosition.y() <= PosY && pos[0])
	{
		BandX = PosX - 1;
		BandY = PosY - 1;
		return;
	...
	if(mouseTilePosition.x() >= PosX && mouseTilePosition.y() <= PosY && pos[1])
	{
		BandX = PosX;
		BandY = PosY - 1;
		return;
	...
	if(mouseTilePosition.x() >= PosX && mouseTilePosition.y() >= PosY && pos[2])
	{
		BandX = PosX;
		BandY = PosY;
		return;
	...
	if(mouseTilePosition.x() <= PosX && mouseTilePosition.y() >= PosY && pos[3])
	{
		BandX = PosX - 1;
		BandY = PosY;
		return;

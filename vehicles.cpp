...
void cVehicle::Draw(SDL_Rect *dest){
  ...
  if(OffX)ox=OffX/(64.0/game->hud->Zoom);
  if(OffY)oy=OffY/(64.0/game->hud->Zoom);
  tmp=*dest;
  tmp.x+=ox;
  tmp.y+=oy;
...
//      scr.x=scr.h*((game->Frame%(typ->overlay->w/scr.h)));
      if(Disabled){
        ...
        scr.x=(typ->overlay_org->h*((game->Frame%(typ->overlay->w/scr.h))))/(64.0/game->hud->Zoom);
      }
      if(StartUp&&Alpha){
...
void cVehicle::DrawPath(void){
  ...
  if(!mjob||!mjob->waypoints||owner!=game->ActivePlayer){
    ...
    while(mx!=BandX||my!=BandY){
      dest.x=180-game->hud->OffX/(64.0/zoom)+zoom*mx;
      dest.y=18-game->hud->OffY/(64.0/zoom)+zoom*my;
      dest.w=zoom;
      dest.h=zoom;
  ...
  zoom=game->hud->Zoom;
  dest.x=180-game->hud->OffX/(64.0/zoom)+zoom*PosX;
  dest.y=18-game->hud->OffY/(64.0/zoom)+zoom*PosY;
  dest.w=zoom;
  dest.h=zoom;
  wp=mjob->waypoints;
...
void cVehicle::DrawMunBar(void){
  ...
  r2.y=(r1.y=game->hud->Zoom-r1.h+GetScreenPosY())+1;
...
void cVehicle::ShowTransfer(sGameObjects *target){
  ...
  ScaleSurfaceAdv2(typ->img_org[0],typ->img[0],typ->img_org[0]->w*(game->hud->Zoom/64.0),typ->img_org[0]->h*(game->hud->Zoom/64.0));
  if(pv){
...
void cVehicle::DrawCommandoCursor(struct sGameObjects *go,bool steal){
  ...
  r.w=35*(CalcCommandoChance(steal)/100.0);

...
void cGameGui::connectToClient (cClient& client)
{
	...
	clientSignalConnectionManager.connect (client.unitStartedWorking, [&](const cUnit& unit)
	{
		...
		{
			stopSelectedUnitSound ();
			if (unit.data.ID.isABuilding ()) soundManager->playSound (std::make_shared<cSoundEffectUnit>(eSoundEffectType::EffectStartWork, static_cast<const cBuilding&>(unit).uiData->Start, unit));
	...
	clientSignalConnectionManager.connect (client.unitStoppedWorking, [&](const cUnit& unit)
	{
		...
		{
			stopSelectedUnitSound ();
			if (unit.data.ID.isABuilding ()) soundManager->playSound (std::make_shared<cSoundEffectUnit> (eSoundEffectType::EffectStopWork, static_cast<const cBuilding&>(unit).uiData->Stop, unit));
			updateSelectedUnitIdleSound ();
		}
	});

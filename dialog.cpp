...
bool ShowYesNo(string text){
  ...
  if(Alpha){
    SDL_BlitSurface(gfx_shadow,NULL,buffer,NULL);
  }
  LoadPCXtoSF((char *) Dialog2Path.c_str(),gfx_dialog);
  dest.x=640/2-300/2;
  dest.y=480/2-231/2;
  dest.w=300;
  dest.h=231;
  SDL_BlitSurface(gfx_dialog,NULL,buffer,&dest);
  ...
  LoadPCXtoSF((char *) DialogPath.c_str(),gfx_dialog);
  game->fDrawMap=true;
  return ret;
}
...
int ShowNumberInput(string text){
  ...
  LoadPCXtoSF((char *) Dialog3Path.c_str(),gfx_dialog);
  dest.x=640/2-300/2;
  dest.y=480/2-231/2;
  dest.w=300;
  dest.h=231;
  ...
  while(1){
    ...
    if(DoKeyInp(keystate)||timer2){
      ...
      dest.x=20+170;
      dest.y=167+124;
      SDL_BlitSurface(gfx_dialog,&scr,buffer,&dest);

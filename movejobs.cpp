...
void cServerMoveJob::doEndMoveVehicle()
{
	...
	{
		server->addAttackJob(mine, Vehicle->PosX, Vehicle->PosY);
		bEndForNow = true;
	}
...
void cEndMoveAction::executeAttackAction (cServer& server)
{
	...
	server.addAttackJob(vehicle_, x, y);
}
...
#define TESTXY_DP(a, b) if (Dest.x a LastDest->x && Dest.y b LastDest->y)
	...
	if (bSpezial)
	{
		SDL_BlitSurface (OtherData.WayPointPfeileSpecial[iIndex][64 - client->getGameGUI().getTileSize()].get(), NULL, cVideo::buffer, &Dest);
	}
	else
	{
		SDL_BlitSurface (OtherData.WayPointPfeile[iIndex][64 - client->getGameGUI().getTileSize()].get(), NULL, cVideo::buffer, &Dest);
	}
}

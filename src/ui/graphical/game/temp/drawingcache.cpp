...
void sDrawingCacheEntry::init (const cVehicle& vehicle, const cMapView& map, const cPlayer* player, unsigned long long animationTime, double zoom_, unsigned long long frameNr)
{
	...
	int height = narrow_cast<int> (std::max (uiData->img_org[toUnderlyingType (vehicle.dir)]->h * zoom, uiData->shw_org[toUnderlyingType (vehicle.dir)]->h * zoom));
	int width = narrow_cast<int> (std::max (uiData->img_org[toUnderlyingType (vehicle.dir)]->w * zoom, uiData->shw_org[toUnderlyingType (vehicle.dir)]->w * zoom));
	if (vehicle.getFlightHeight() > 0)
	{
		int shwOff = narrow_cast<int> (Round (narrow_cast<float> (uiData->img_org[toUnderlyingType (vehicle.dir)]->w * zoom)) * (vehicle.getFlightHeight() / 64.0f));
		height += shwOff;
		width += shwOff;
	}

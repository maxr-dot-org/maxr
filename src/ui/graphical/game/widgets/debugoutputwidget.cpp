...
void cDebugOutputWidget::draw (SDL_Surface& destination, const cBox<cPosition>& clipRect)
{
	...
	if (debugFX)
	{
		if (gameMap)
		{
			...
			const auto runningAnimations = std::ranges::count_if (gameMap->animations, [] (const std::unique_ptr<cAnimation>& animation) { return animation->isRunning(); });
			print ("running-animations-count: " + std::to_string (runningAnimations));
			const auto finishedAnimations = std::ranges::count_if (gameMap->animations, [] (const std::unique_ptr<cAnimation>& animation) { return animation->isFinished(); });
...
void cDebugOutputWidget::traceVehicle (const cVehicle& vehicle, cPosition& drawPosition)
{
	...
	tmpString = (vehicle.bandPosition ? " band: " + toString (*vehicle.bandPosition) : "") + (vehicle.buildBigSavedPosition ? " build_big_saved_pos: " + toString (*vehicle.buildBigSavedPosition) : "");
	font->showText (drawPosition, tmpString, eUnicodeFontType::LatinSmallWhite);
	drawPosition.y() += 8;

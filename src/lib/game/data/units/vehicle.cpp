...
bool cVehicle::canLand (const cMap& map) const
{
	...
	const auto b_it = std::ranges::find_if (buildings, [] (const auto* b) { return b->getStaticData().canBeLandedOn; });
	...
	if (std::ranges::any_of (v, [&] (const cVehicle* vehicle) { return vehicle->getFlightHeight() < 64 && vehicle->iID != iID; })) return false;

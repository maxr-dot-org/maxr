...
#ifndef utility_signal_novariadic_signal_2H
#define utility_signal_novariadic_signal_2H

template<typename R, typename Arg1, typename Arg2, typename MutexType, typename ResultCombinerType>
class cSignal<R (Arg1, Arg2), MutexType, ResultCombinerType> : public cSignalBase
{
	typedef cSlot<R (Arg1, Arg2)> SlotType;
	typedef std::list<SlotType> SlotsContainerType;
	typedef std::tuple<Arg1, Arg2> ArgumentsContainerType;
...
public:
	...
	template<typename Args21, typename Args22>
	result_type operator() (Args21&& arg1, Args22&& arg2)
	{
		std::unique_lock<cMutex> lock (mutex);

		auto arguments = ArgumentsContainerType (std::forward<Args21> (arg1), std::forward<Args22> (arg2));
...
#endif // utility_signal_novariadic_signal_2H

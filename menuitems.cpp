...
{
	return overItem(cPosition(x,y));
}

bool cMenuItem::overItem (const cPosition& pos) const
{
	if(pos.x() >= position.x && pos.x() < position.x + position.w &&
	   pos.y() >= position.y && pos.y() < position.y + position.h)
	{
		return true;
	}
	return false;
...
void cMenuItemContainer::clicked (void* parent)
{
	...
	for (unsigned int i = 0; i < itemList.size(); i++)
	{
		if (itemList[i]->overItem (cMouse::getInstance().getPosition())) itemList[i]->clicked (this);
	}
}
...
void cMenuItemContainer::released (void* parent)
{
	for (unsigned int i = 0; i < itemList.size(); i++)
	{
		if(itemList[i]->overItem(cMouse::getInstance().getPosition())) itemList[i]->released(this);
...
void cMenuItemContainer::hoveredOn (void* parent)
{
	for (unsigned int i = 0; i < itemList.size(); i++)
	{
		if(itemList[i]->overItem(cMouse::getInstance().getPosition())) itemList[i]->hoveredOn(this);
	}
}
...
void cMenuItemContainer::movedMouseOver (int lastMouseX, int lastMouseY, void* parent)
{
	for (unsigned int i = 0; i < itemList.size(); i++)
	{
		if(itemList[i]->overItem(lastMouseX, lastMouseY) && !itemList[i]->overItem(cMouse::getInstance().getPosition())) itemList[i]->hoveredAway(this);
		else if(!itemList[i]->overItem(lastMouseX, lastMouseY) && itemList[i]->overItem(cMouse::getInstance().getPosition())) itemList[i]->hoveredOn(this);
		else if(itemList[i]->overItem(lastMouseX, lastMouseY) && itemList[i]->overItem(cMouse::getInstance().getPosition())) itemList[i]->movedMouseOver(lastMouseX, lastMouseY, this);
	}
}
...
bool cMenuRadioGroup::overItem (const cPosition& position) const
{
	for (unsigned int i = 0; i < buttonList.size(); i++)
	{
		if (buttonList[i]->overItem (position))
			return true;
	}
	return false;
}
...
void cMenuRadioGroup::clicked (void* parent)
{
	for (unsigned int i = 0; i < buttonList.size(); i++)
	{
		if(buttonList[i]->overItem(cMouse::getInstance().getPosition()))
		{
...
void cMenuUnitsList::released (void* parent)
{
	...
	{
		if (i >= (int) unitsList.size()) break;
		if(unitsList[i]->overItem(cMouse::getInstance().getPosition()))
		{
...
void cMenuUpgradeHandler::buttonReleased (void* parent)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if(This->increaseButtons[i]->overItem(cMouse::getInstance().getPosition()))
		{
		...
		else if(This->decreaseButtons[i]->overItem(cMouse::getInstance().getPosition()))
		{
...
{
	mouseXOff = cMouse::getInstance().getPosition().x() - position.x;
	mouseYOff = cMouse::getInstance().getPosition().y() - position.y;
	return true;
}
...
void cMenuScroller::mouseMoved (bool center)
{
	...
	{
		case SCROLLER_TYPE_VERT:
			position.y = cMouse::getInstance().getPosition().y() - (center ? (position.h / 2) : mouseYOff);
			break;
		case SCROLLER_TYPE_HUD_ZOOM:
		case SCROLLER_TYPE_HORI:
			position.x = cMouse::getInstance().getPosition().x() - (center ? (position.w / 2) : mouseXOff);
			break;
	}
...
bool cMenuLineEdit::preClicked()
{
	if (active)
	{
		int x = cMouse::getInstance().getPosition().x() - (position.x + getTextDrawOffset().x);
...
bool cMenuLineEdit::handleKeyInput (cKeyboard&, SDL_Keycode key, cMenu* parent)
{
...
void cMenuLineEdit::handleTextInputEvent(cKeyboard& keyboard, const char* inputText, cMenu* parent)
{
...
{
	if(cMouse::getInstance().getPosition().x() > position.x + position.w - 17 - 15 && cMouse::getInstance().getPosition().x() < position.x + position.w - 17 - 5)
	{
		for (int i = scrollBar->offset; i < scrollBar->offset + maxDrawPlayers; i++)
		{
			if (i >= (int) players->size()) break;
			if(cMouse::getInstance().getPosition().y() > position.y + 12 + 14 * (i - scrollBar->offset) && cMouse::getInstance().getPosition().y() < position.y + 12 + 14 * (i - scrollBar->offset) + 10)
			{
...
{
	int clickedIndex = (cMouse::getInstance().getPosition().y() - position.y - 20) / 55 + index * maxItems;
